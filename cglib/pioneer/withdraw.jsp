...
<div class="main cs">
	...
	<form action="pioneer_withdraw_deposit.shtml"  method="post" class="fukrm fukrm-ajax">
		...
		<cite>
			...
			<c:if test="${withdraw_type eq 1}">
				<strong style="display:inline-block;color:#f00;font-size:50px;" class="allmoney">${balance_recommend}</strong>元
			</c:if>
			<c:if test="${withdraw_type eq 2}">
				<strong style="display:inline-block;color:#f00;font-size:50px;" class="allmoney">${balance_position}</strong>元
			</c:if>
			<c:if test="${withdraw_type eq 3}">
				<strong style="display:inline-block;color:#f00;font-size:50px;" class="allmoney">${balance_manager}</strong>元
			</c:if>
			<c:if test="${withdraw_type eq 4}">
				<strong style="display:inline-block;color:#f00;font-size:50px;" class="allmoney">${sum_bonus}</strong>元
			</c:if>
		</cite>
		...
		<cite>
			...
			<input type="text" name="takemoney" placeholder="请输入提取金额" class="takemoney" value="500" check="required,number,between" min="500" max="50000" >
		</cite>
		...
		<cite>
			...
			<p><a href="/cglib/pioneer/password.jsp" style="color:#08c;">修改提现密码？</a>每日密码输入错误超过三次资金冻结一天,24小时后解封</p>
		</cite>
