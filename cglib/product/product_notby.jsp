	...
	<div class="main cs">
		...
		<div class="aside">
			<dl>
				...
				<dd class="active"><a href="product_Notthroughlist_request.shtml"><i class="icon icon-shenhebutongguo"></i>未通过商品</a></dd>
		...
		<h2 class="tle"><i class="icon icon-shenhebutongguo"></i>未通过商品<small>您的商品未通过审核，请修改后重新提交审核</small></h2>

		<form action="productName_Notthroughlist_request.shtml" class="fukrm table_tle" method="post">
			商品名：<input type="text" name="wname" value="${wname }" style="width:100px;" maxlength="20">
		...
		<div class="table">
			<table border="0" cellspacing="0" cellpadding="0">
				...
				<c:forEach var="list" items="${pageSupport.result}" varStatus="index" begin="${pageSupport.currentPageNo*pageSupport.pageSize-pageSupport.pageSize}" end="${pageSupport.currentPageNo*pageSupport.pageSize-1}">
					<tr>
						...
						<td>${list.brand}</td>
						<td>${list.tid}</td>
						<td><span style="width: 68px;height: 21px;line-height: 21px;background-color:#E24B62;text-align: center;display: block;color: #ffffff;border-radius:6px;">未通过</span></td>
						<td>
			...
			<div class="ajaxpage" total="${pageSupport.totalRecordCount}" each="${pageSupport.pageSize}" page="${pageSupport.currentPageNo}" ajax="false" url="product_Notthroughlist_request.shtml"></div>  
