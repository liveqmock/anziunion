...
.yl:HOVER{ text-decoration: underline;color: red;}
	...
	<div class="main cs">
		...
		<h2 class="tle"><i class="icon icon-xiajiaguanli"></i>已下架商品<small>已下架商品修改后还可以上架哦</small></h2>
		<form action="productName_uplist_request.shtml" class="fukrm table_tle" method="post">
			商品名：<input type="text" name="xname" value="${xname }" style="width:100px;" maxlength="20">
		...
		<div class="table">
			<table border="0" cellspacing="0" cellpadding="0">
				...
				<c:forEach var="list" items="${pageSupport.result}" varStatus="index" begin="${pageSupport.currentPageNo*pageSupport.pageSize-pageSupport.pageSize}" end="${pageSupport.currentPageNo*pageSupport.pageSize-1}">
					<tr>
						...
						<td>${list.brand}</td>
						<td>${list.tid}</td>
												<td><span style="width: 68px;height: 21px;line-height: 21px;background-color:#CDB4B8;text-align: center;display: block;color: #ffffff;border-radius:6px;">已下架</span></td>
						<td>
							<a class="color_red " onclick="return confirm('确定修改?')" href="getUpdateManagement.shtml?id=${list.id}" >修改</a>
			...
			<div class="ajaxpage" total="${pageSupport.totalRecordCount}" each="${pageSupport.pageSize}" page="${pageSupport.currentPageNo}" ajax="false" url="product_downlist_request.shtml"></div>  
