	...
	<div class="main cs">
		...
		<div class="aside">
			<dl>
				...
				<dd class="active"><i class="icon icon-76wodeyibanyewu"></i><a href="goods_publish.shtml">发布管理</a></dd>
			</dl>
		</div>
			...
			<table border="0" cellspacing="0" cellpadding="0">
				...
				<c:forEach var="order_union_list" items="${pageSupport.result}" varStatus="status" begin="${pageSupport.currentPageNo*pageSupport.pageSize-pageSupport.pageSize}" end="${pageSupport.currentPageNo*pageSupport.pageSize-1}">
					 <tr <c:if test="${status.index%2 != '1'}">style="background-color:#fafafa"</c:if>>
						...
						<td height="90px">
							<span class="btn btn_blue">发布</span>
						</td >
						<td height="90px">
							<a class="btn btn_red js-shelves" href="cancel_publish.shtml?gid= ${order_union_list.id}">取消发布</a>
							<%--<a class="btn btn_greed js-data" href="data.shtml?gid= ${order_union_list.id}">数据</a> --%>
						</td>
					</tr>
             </c:forEach>
				</tbody>
		...
		$(function(){
			...
			dialog.on('submit','.dialog_straddling',function(){
				...
				if(this.submited){
					...
					var val=$('input:radio[name="good_id"]:checked').val();
					if(val==null){
						alert("未选择任何商品！");
						return false;
			...
			dialog.on('click','.dialog_straddling_shelves',function(){
				$.ajax({
					...
					success: function(data) { 
						 ...
						 dialog.on( "dialogbeforeclose", function() {location.reload();});
					}
				});
				return false;
			})
			...
			$(document.body).on('click','a.js-data',function(){
				$.ajax({
					...
					success: function(data) { 
						...
						var html = '<div class="dialog_straddling" style="width:300px;"><h2>商品数据</h2>发布时间：'+data.good_pubdate;
						if(data.pub_user==data.uid){
					
						       html=html+'<br>销售商家：'+data.user_num+'&nbsp&nbsp&nbsp&nbsp&nbsp已售件数：'+data.total_money/data.pice+'<br>营业额：'+data.pice*data.user_num+'<br>提成比列：'+data.profit+'%<br>提成分润：'+data.pice*data.user_num*data.profit/100+'<br><br><div style="text-align: center;"><b class="cancel">关闭</b></div></div>';
						dialog.html(html).dialog('open');
						}
						else{
						html=html+'<br>已售件数：'+data.ordernum+'<br>营业额：'+data.pice*data.user_num+'<br>提成比列：'+data.profit+'%<br>提成利润：'+data.pice*data.ordernum*data.profit/100+'<br><br><div style="text-align: center;"><b class="cancel">关闭</b></div></div>';
						dialog.html(html).dialog('open');
						}
			...
			// 确认下架
				...
				var html = '<div class="dialog_straddling" style="width:300px;"><h2>是否取消发布？</h2><br><div style="text-align: center;"><a class="dialog_straddling_shelves btn_submit" href="'+this.getAttribute('href')+'">确定取消发布</a><b class="cancel">关闭</b></div></div>';
					dialog.html(html).dialog("option","title", "取消发布商品").dialog('open');
